                    ...
                    <form onSubmit={handleSubmit} style={styles.form}>
                        ...
                        <div style={styles.sellerRow}>
                            <label htmlFor="isSeller" style={styles.sellerLabel}>
                                <div style={{
                                    ...styles.toggle,
                                    ...(formData.isSeller ? styles.toggleActive : {})
                                }}>
                                    <div style={{
                                        ...styles.toggleKnob,
                                        ...(formData.isSeller ? styles.toggleKnobActive : {})
                                    }}></div>
                                </div>
...
// We need to show the left panel on desktop. Since inline styles can't do media queries,
// we'll use a responsive wrapper approach. We handle it via CSS-in-JS override.
const responsiveCSS = document.createElement('style');
responsiveCSS.textContent = `
    @media (min-width: 1024px) {
        .ve-left-panel { display: flex !important; }
        .ve-mobile-logo { display: none !important; }
    }
    @media (max-width: 640px) {
        .ve-right-panel { padding: 28px 24px !important; }
    }
`;
if (!document.getElementById('ve-register-responsive')) {
    responsiveCSS.id = 've-register-responsive';
    document.head.appendChild(responsiveCSS);
}

// Patching: assign classNames so CSS media queries work
const OriginalRegister = Register;
const ResponsiveRegister = () => {
    return <OriginalRegister />;
};

// We re-export Register but we need to apply className patches inside the component.
// Let's restructure to use className-aware approach:
